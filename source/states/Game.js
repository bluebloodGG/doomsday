...
Doomsday.Main.prototype.create = function() {
	...
	this.map.setCollision([1, 2, 3, 4, 6, 7, 8, 9, 11, 12, 13, 16, 17, 18]);
	...
	this.monsterManager = new MonsterManager(this.game, this.player.soldier.torso);
	this.monsterManager.generateMonsters(100);

	this.player.soldier.torso.bringToTop();
	for (var i = 0; i < 0; i++) {
		this.zombies.push(new Enemy(game, player));
	}	
};
...
Doomsday.Main.prototype.update = function() {        
	...
	this.game.physics.arcade.collide(this.player.soldier.torso, this.layer);
};
